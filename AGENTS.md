...
- **Tests must be real**: real Postgres, real Docker, real AI calls via the free `opencode/big-pickle` model. No fakes as the only coverage of any surface. Fakes may exist for dev ergonomics but every faked surface must also have a real-backend counterpart.
