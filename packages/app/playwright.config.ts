...
// Both the roomy-server (under test) and the Vite preview server are
// started by globalSetup, because Playwright's built-in webServer starts
// BEFORE globalSetup — which means the ROOMY_API_URL env var the Vite
// proxy config reads would be unset and it would proxy to the developer's
// local :3000. See e2e/fixtures/global-server.ts.
const VITE_PORT = Number(process.env.ROOMY_E2E_VITE_PORT ?? 5179);
