...
/**
 * Optimistically drop a chat attachment from every cached
 * `getChatArtifacts` entry for a chat so the Files sidebar updates the
 * instant the user confirms removal, instead of waiting for the
 * invalidation refetch. Patches all arg variants (showHidden /
 * includeArtifacts) by walking the cache. Returns undo handles so the
 * caller can roll back if the server DELETE fails.
 */
function removeChatAttachmentFromCaches(
  dispatch: (action: unknown) => unknown,
  getState: () => unknown,
  chatId: string,
  name: string,
): Array<{ undo(): void }> {
  const isTarget = (f: ServerFile) => f.name === name || f.path.split("/").pop() === name;
  const drop = (draft: ServerFile[]) => {
    for (let i = draft.length - 1; i >= 0; i--) {
      if (isTarget(draft[i])) draft.splice(i, 1);
    }
  };
  const undos: Array<{ undo(): void }> = [];
  const state = getState() as Record<string, unknown>;
  const apiState = state[api.reducerPath] as { queries?: Record<string, { data?: ServerFile[] }> } | undefined;
  if (!apiState?.queries) return undos;
  for (const [key, entry] of Object.entries(apiState.queries)) {
    if (!key.startsWith("getChatArtifacts(")) continue;
    if (!entry?.data?.some(isTarget)) continue;
    // Only patch entries that belong to this chat — the cache key encodes
    // the args, so a quick substring check avoids parsing every variant.
    if (!key.includes(`"chatId":"${chatId}"`)) continue;
    const argJson = key.slice("getChatArtifacts(".length, -1);
    try {
      undos.push(dispatch(api.util.updateQueryData("getChatArtifacts", JSON.parse(argJson) as { chatId: string; showHidden?: boolean; includeArtifacts?: boolean }, drop)) as { undo(): void });
    } catch {
      // Cache keys are generated by RTK Query; if that format changes,
      // skipping one optimistic patch is safer than failing the delete.
    }
  }
  return undos;
}
...
export const api = createApi({
  ...
  endpoints: (build) => ({
    ...
    >({
      ...
      // Optimistically remove the row from the Files sidebar so it
      // disappears on confirm; roll back if the server rejects. The tag
      // invalidation below still refetches to reconcile with the server.
      async onQueryStarted({ chatId, name }, { dispatch, getState, queryFulfilled }) {
        const undos = removeChatAttachmentFromCaches(dispatch, getState, chatId, name);
        try {
          await queryFulfilled;
        } catch {
          for (const u of undos) u.undo();
