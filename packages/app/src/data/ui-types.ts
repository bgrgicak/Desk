...
export interface Task {
   ...
   *  (picked up by the user or the AI / a run is executing / parent
   *  state is `running`). `needs_input` = the AI paused awaiting the
   *  user's reply (the AI moves it here and back). `scheduled` = has a
   *  future run. `complete` = done/cancelled. `failed` = a run or task
   *  terminated in error and no retry has succeeded since. */
