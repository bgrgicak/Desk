  ...
  return (
    <div className="rounded-2xl bg-background/95 backdrop-blur-sm border border-border/60 shadow-2xl px-8 py-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col items-center gap-5">
        <RoomyLogo className="h-7 w-auto text-foreground" />
      ...
      {/* Sign-up link — live when ROOMY_ENABLE_SIGNUP=1 on the server,
      ...
      {signupEnabled && onSignUp ? (
        ...
        <p className="text-center text-sm text-muted-foreground" data-testid="signup-coming-soon">
          Don&apos;t have an account?{' '}
          <span className="text-muted-foreground/70 font-medium" title="Account signup is disabled — set ROOMY_ENABLE_SIGNUP=1 on the server to enable">
            Sign up — coming soon
          </span>
        </p>
      )}
...
function RoomyLogo({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 58 21"
      fill="currentColor"
