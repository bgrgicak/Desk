...
Built-in Desk app. Provides fragments that agents attach to a chat message
...
Synced into `~/Desk/.apps/chat-forms.app/` on server start by
`runtime.writeBuiltinApps()`, then mounted read-only at
`/opt/desk-apps/chat-forms.app/` inside every sandbox.
...
6. Register the name in `desk.app.json` and rebuild.
