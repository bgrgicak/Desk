...
describe("renderAgentFile", () => {
  it("generates a plain markdown body (pi reads AGENTS.md as text, no frontmatter)", () => {
    ...
    // Identity framing
    expect(result).toContain("You are Jarvis, call me Roomy.");
    ...
    // Roomy reference manuals are skills, not inlined prompt text.
    expect(result).toContain("## Roomy native skills");
    expect(result).toContain("roomy-cli-task-schedule");
    expect(result).toContain("roomy-agent task schedule");
    expect(result).not.toContain("# Roomy CLI");
  ...
  it("renders the user's timezone in the scheduling section", () => {
    ...
    expect(result).toContain("America/Los_Angeles (Roomy's app client)");
  ...
  it("tells the agent to assume UTC + mention it when timezone is unknown", () => {
    ...
    expect(result).toContain("`roomy-agent task schedule`");
    expect(result).toContain("`roomy-agent task reschedule`");
    expect(result).toContain("`roomy-agent task cancel`");
  });
  ...
  it("renders a narrow summary-only prompt for summary runs", () => {
    ...
    expect(result).toContain("Roomy stores the final markdown as a `summary` message");
    expect(result).toContain("Chat summaries: ~/.chats/cht_abc/notes/");
    expect(result).not.toContain("## Your workspace");
    expect(result).not.toContain("roomy-agent chat attach-artifact");
  ...
  it("keeps the base prompt concise when no goal or chat is set", () => {
    ...
    expect(result).toContain("Your mandate is to help Roomy accomplish their goals");
  ...
  it("renders goal and chat paths before the Roomy skill router", () => {
    const result = renderAgentFile({
      agentId: "agt_doc",
      agentName: "Helper",
      model: "anthropic/claude-haiku-4-5",
    ...
    expect(result.indexOf("## User's goal: write a document"))
      .toBeLessThan(result.indexOf("## Roomy native skills"));
  });
});
...
describe("writeWorkspaceMcpConfig", () => {
  it("writes the playwright entry with enabled=true and the right argv for browser-goal chats", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "roomy-mcp-cfg-"));
    try {
  ...
  it("emits enabled=false explicitly when the chat doesn't need a browser so a previously-enabled entry gets re-disabled", async () => {
    // Without an explicit `enabled: false` the roomy-mcp-bridge extension
  ...
  it("returns changed=false when the file already has the desired content (the per-workspace lock dedupes)", async () => {
    ...
    // Roomy doesn't overwrite it. (This sticky-on behavior is intentional
  ...
  it("user-owned playwright entry (enabled=true in mcp.json) wins over the goal-gated managed config", async () => {
    ...
    // playwright.enabled=true is telling Roomy "I want browser tools
    // in every chat, regardless of goal". Roomy respects that and
  ...
  it("resets a user's enabled=false playwright entry to managed (no opt-in signal → goal-gate applies)", async () => {
    ...
    // garbage) leave Roomy in charge — otherwise a stale Roomy-written
