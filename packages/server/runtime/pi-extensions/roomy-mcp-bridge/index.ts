/**
 * roomy-mcp-bridge — pi extension that proxies arbitrary MCP servers'
 ...
 * Roomy we need MCP because every browser-goal chat already depends on
...
async function loadConfig(cwd: string): Promise<McpBridgeConfig> {
  ...
  try {
    ...
    process.stderr.write(`[roomy-mcp-bridge] failed to read ${p}: ${(err as Error).message}\n`);
    return {};
  }
}
...
class McpClient {
  ...
  async connect(cmd: string, args: string[], env: Record<string, string>): Promise<void> {
    ...
    this.child.stderr.on("data", (chunk: Buffer) => {
      ...
      process.stderr.write(`[roomy-mcp-bridge:${this.serverName}] ${chunk.toString().trimEnd()}\n`);
    });
    ...
    await this.request("initialize", {
      ...
      capabilities: { tools: {} },
      clientInfo: { name: "roomy-mcp-bridge", version: "0.1.0" },
    });
  ...
  private onData(chunk: string): void {
    ...
    while ((nl = this.buffer.indexOf("\n")) >= 0) {
      ...
      try {
        ...
        process.stderr.write(`[roomy-mcp-bridge:${this.serverName}] non-JSON line: ${line.slice(0, 200)}\n`);
        continue;
      }
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    for (const [name, server] of Object.entries(servers)) {
      ...
      try {
        ...
        process.stderr.write(`[roomy-mcp-bridge] failed to start '${name}': ${(err as Error).message}\n`);
      ...
      try {
        ...
        process.stderr.write(`[roomy-mcp-bridge] tools/list failed for '${name}': ${(err as Error).message}\n`);
        client.close();
        clients.delete(name);
        allSpawned.delete(client);
        continue;
