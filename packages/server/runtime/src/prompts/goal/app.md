...
The user is building a small Roomy app: a tool, tracker, dashboard, calculator,
...
1. Before scaffolding, offering to build, or saying an app does not exist,
   search the current workspace library with `roomy-agent find library`.

   If a matching app or fragment satisfies the request, reuse it and attach it
   immediately with `roomy-agent chat attach-artifact`; do not scaffold, rebuild,
...
2. Scaffold once per app:
   ...
   ```sh
   roomy-agent app create --chat <chatId> <name>
   ```

   Use a kebab-case name. This creates
   `~/.chats/<chatId>/artifacts/<name>.app/` from the Roomy app scaffold with
   Vite, fragments, storage client, `AGENTS.md`, and installed dependencies.
   Do not hand-roll another project structure.

3. Load `roomy-app-scaffold` before editing the app. Follow its static-only,
   ...
   constants, or ad-hoc JSON as the source of truth. Roomy storage is what lets
   app data move across clients such as desktop and phone when Roomy syncs app
   storage.
...
4. Design fragments first, then compose the app:
   ...
   - Implement feature behavior in fragment components whenever it can stand on
     its own as a Roomy surface, be reused by another app, or has its own
...
5. Iterate in place. Update `roomy.app.json` when adding or removing fragments
   ...
   example fragment in `roomy.app.json`, or leave a generated example surface in
   the built output.
...
6. Before saying the app is ready, run `npm run build` from the app directory
   ...
   as fully verified. Manually test the built app in Roomy's sandboxed iframe,
   ...
   `getStorageClient()` or `window.roomy.storage`. Vitest can use fabricated
...
7. Surface built or reused app updates with:
   ...
   ```sh
   roomy-agent chat attach-artifact --chat <chatId> <name>.app
   ```
...
Roomy apps are static client-side bundles. They must not embed servers, auth,
background jobs, or direct Roomy API calls. If the user asks for something that
