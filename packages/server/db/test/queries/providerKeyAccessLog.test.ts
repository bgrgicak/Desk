...
describe("providerKeyAccessLog queries", () => {
  it("logKeyAccess inserts a row and getKeyAccessLog returns it", async () => {
    ...
    await providerKeyAccessLog.logKeyAccess(
      pool, userId, "write", ["ANTHROPIC_API_KEY"], "user_update",
    );
    ...
    expect(log[0].providers).toEqual(["ANTHROPIC_API_KEY"]);
  ...
  it("getKeyAccessLog returns entries newest-first", async () => {
    ...
    await providerKeyAccessLog.logKeyAccess(pool, userId, "read", ["ANTHROPIC_API_KEY"], "sandbox_run:msg_1");
  ...
  it("getKeyAccessLog is scoped per user", async () => {
    ...
    await providerKeyAccessLog.logKeyAccess(pool, userA, "write", ["ANTHROPIC_API_KEY"], "user_update");
  ...
  it("logKeyAccess accepts null reason", async () => {
    ...
    await providerKeyAccessLog.logKeyAccess(pool, userId, "read", ["ANTHROPIC_API_KEY"]);
    const log = await providerKeyAccessLog.getKeyAccessLog(pool, userId);
  ...
  it("getKeyAccessLog respects limit", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await providerKeyAccessLog.logKeyAccess(pool, userId, "read", ["ANTHROPIC_API_KEY"], `sandbox_run:msg_${i}`);
    }
