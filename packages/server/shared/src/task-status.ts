 ...
 *   2. Terminal failure → Failed. Sticky until retried.
 *   3. Any child task_run currently running → Active.
 *   4. Hosting chat has a running agent_turn → Active.
 *   5. Scheduled (executeAt / cron set) → Scheduled.
 *   6. Hosting chat is unread → Needs input.
 *   7. Parent state='running' → Active. (Sandbox auto-fire keeps the
