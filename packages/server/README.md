...
- [storage/](storage/) — file layout under `~/Roomy/`
...
- [sandbox-cli/](sandbox-cli/) — the `roomy-agent` binary installed inside sandboxes
...
[`packages/server/docs/roomy-api.postman_collection.json`](docs/roomy-api.postman_collection.json)
covers every route in [`api/src/app.ts`](api/src/app.ts).

## Troubleshooting

- **`roomy-server` exits immediately**: check `~/Roomy/roomy.db` for stale
...
- **`No container runtime available` on boot**: the runtime probes
  ...
  or set `ROOMY_CONTAINER_ENGINE=docker|nerdctl` if both are present and
...
- **Sandbox bind-mount writes fail under a rootless runtime**: the
  ...
  `ROOMY_SANDBOX_USER=0:0` to pin the override.
