...
function requestMultipart(
  ...
  return new Promise((resolve, reject) => {
    const boundary = `----roomy-ws-${Math.random().toString(16).slice(2)}`;
    const chunks: Buffer[] = [];
...
beforeAll(async () => {
  const dbDir = await fs.mkdtemp(path.join(os.tmpdir(), "roomy-ws-scope-db-"));
  dbPath = path.join(dbDir, "test.sqlite3");
  pool = new Pool({ path: dbPath });
  await runMigrations(pool);

  home = await fs.mkdtemp(path.join(os.tmpdir(), "roomy-ws-scope-"));
