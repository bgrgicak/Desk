...
import { findArtifacts } from "../src/routes/search.js";
...
beforeAll(async () => {
  dbDir = await fs.mkdtemp(path.join(os.tmpdir(), "desk-find-artifacts-db-"));
...
describe("findArtifacts", () => {
  it("discovers fragment manifests with params_schema via the existing file index", async () => {
    ...
    const hits = await findArtifacts(pool, { pool, home }, userId, {
      query: "inline list",
      kind: "fragment",
      workspaceId,
    });
  ...
  it("returns recent notes when query is omitted", async () => {
    ...
    const hits = await findArtifacts(pool, { pool, home }, userId, { workspaceId });
