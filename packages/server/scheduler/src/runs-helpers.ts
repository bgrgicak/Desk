 ...
 * configured model has no live auth in the current run env. Every run
 * path falls back to it — keeping reflection / chat / summary alive
 * instead of leaving a run in a broken "no auth at all" state. Lives
 * in helpers (not runs.ts) so reflection can use the same resolver
 * without creating a runs ↔ reflection import cycle.
 *
 * Pi-era: `anthropic/claude-haiku-4-5` replaces the pre-pi default of
 * `opencode/big-pickle` because the opencode provider no longer exists
 * after the pi-runtime migration.
 */
