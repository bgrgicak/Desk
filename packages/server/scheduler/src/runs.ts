...
// `resolveFallbackModels` ships with the pi-runtime migration so the
// per-turn pi driver can cycle through alternate model ids when the
// agent's primary fails with a provider-shaped error.
export {
  FALLBACK_MODEL,
  resolveFallbackModels,
  resolveModelForRun,
