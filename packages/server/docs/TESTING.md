...
|---|---|---|
| `DATABASE_URL` or `DESK_TEST_DATABASE_URL` | `sqlite://desk:desk@127.0.0.1:55432/desk` | Admin SQLite connection for creating per-worker test databases |
...
### Integration tests
Hit real backends (SQLite, Docker, `at`/`crontab`). Each test worker gets an isolated database (`desk_<pkg>_test_<worker_id>`). Auto-detected — skipped when the backend is unavailable, not gated by opt-in env vars.

### End-to-end tests
Full server stack: HTTP server against real SQLite, real auth, real WebSocket upgrade, real run lifecycle. The API `e2e.test.ts` exercises login, CRUD, message sending, run triggering, and WebSocket event delivery.
