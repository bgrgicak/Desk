...
|---|---|---|
| `ROOMY_HOME` | `$HOME` | Where the test fixture writes per-run sqlite + workspace dirs (under a `mkdtemp` subdir, not the user's real `~/Roomy`) |
...
2. `Vitest (host suite)` builds the sandbox CLI and `roomy/sandbox:v1`, then runs the host Vitest suite.
3. `Playwright e2e` builds `@roomy-ai/api` and its Nx dependencies, installs Chromium, then runs the UI e2e suite with the fake sandbox driver.
...
1. `mkdtemp` a `ROOMY_HOME` per test.
2. Set `ROOMY_DB_PATH` under it; migrations run automatically.
